...
\subsection{Margin Value Analysis of the connecting rod}

To recap, the objective of Margin Value Analysis is to map local excess margins onto their overall impact on design performance and their benefit for absorbing potential changes.
 
\subsubsection{Metric 1: Local excess margin}
The procedure outlined in Section \ref{localmargin} was followed for the connecting rod network represented in the app model (\texttt{man\_diagram\_conrod.json}). The five margin nodes correspond to: $E_1$ I-section thickness, $E_2$ big-end diameter, $E_3$ small-end diameter, $E_4$ bolt size and $E_5$ cap thickness. The resulting local excess values are:
...
This indicates that the largest local overspecification in this case appears at the I-section thickness margin $E_1$, followed by cap thickness $E_5$ and bolt size $E_4$.

\subsubsection{Metric 2: Adverse impact of excess margin}

The second metric assesses adverse impact of each margin on the selected performance parameters. In the connecting-rod model, two performance parameters were used:
\begin{itemize}
\item $P_1$: total component volume.
\item $P_2$: total component mass.
\end{itemize}

Using the procedure outlined in Section \ref{forwardpass}, the impact matrix for the five margins was:
...
Rows of Equation \ref{impactcalc} correspond to $E_1$--$E_5$ and columns correspond to $P_1$ and $P_2$. In this case, $E_1$ has the strongest adverse performance effect, while $E_5$ has a very small adverse effect.
 
\subsubsection{Metric 3: Benefit of margins for absorbing change}        

Two input parameters were marked as inputs of interest for this case:
\begin{itemize}
\item $N$: engine speed.
\item $P_{max}$: peak cylinder pressure.
\end{itemize}

Applying the procedure of Section \ref{metric3}, the absorbed deterioration values were:
...
The corresponding absorption matrix was:
...
Rows in Equation \ref{eq:absorptionout} correspond to margins $E_1$--$E_5$ and columns correspond to inputs $N$ and $P_{max}$. The result indicates that $E_1$--$E_3$ predominantly contribute to absorbing increases in $P_{max}$, while $E_4$ and $E_5$ contribute to absorbing increases in $N$.

\subsection{Margin Value Plot}

With equal weighting of both performance parameters ($W_1=W_2=1$), the weighted impact values were:
...
With equal weighting of both input parameters ($L_N=L_{P_{max}}=1$), weighted absorption values were:
...
The result suggests that $E_1$ is a strong candidate for redesign attention because it combines the highest local excess and highest adverse impact. By contrast, $E_5$ has high absorption potential with very low adverse impact and may therefore provide relatively high value to retain, depending on strategic priorities for change absorption.

\subsubsection{Identifying nonlimiting margins}

With reference to Section 3.3.3, utilisation was computed as:
\begin{align}
\begin{split}
...
For this case, the utilisation matrix was:
...
This indicates that $E_2$ is fully utilised for absorbing the studied increase in $P_{max}$, whereas $E_1$ has low utilisation for that same input. For the studied increase in $N$, $E_4$ is fully utilised and $E_5$ is highly utilised.

\subsection{Comments regarding verification}\label{verification}

To verify the implementation, the metrics above were obtained directly from the app backend analysis of the connecting-rod network (\texttt{man\_diagram\_conrod.json}) and checked for consistency against the governing equations used in the connected calculation and decision nodes.

Overall, the resulting values are coherent with the network structure: margins linked to pressure-sensitive sizing relations dominate absorption for $P_{max}$, while margins linked to speed/inertial paths dominate absorption for $N$.
