...
%Deteriorations
% FIX: normalise by the BASELINE value (p0), not by Pmax.
% Brahma & Wynn (2020) define deterioration as (Pmax_i - p0) / |p0|.
Deterioration_N    = (N_max     - N)     / abs(N);
Deterioration_Pmax = (P_max_max - P_max) / abs(P_max);
...
% ── Absorption for N ────────────────────────────────────────────────────
% FIX: compute thresholds at N_max (decisions held fixed), then use the
%      CHANGE in threshold (threshold_new - threshold_base) normalised by
%      (threshold_base * deterioration).  The _new values must NOT be
%      zeroed before they are used.
F_p_abs = pi*(D_p.^2)/4 .* P_max;   % F_p stays at baseline P_max
[t_req_Nmax, ~,~,~,~,~, n_Nmax, r_Nmax] = t_req_rankine(F_p_abs, sigma_yp, S, K, lc, A_t, Ixx_coeff, H_R, B_R, N_max, N_prime, l_s, rho);
[d_breq_Nmax, ~]   = big_end(F_p_abs, P_bb, l_db);
[d_sreq_Nmax, ~]   = small_end(F_p_abs, P_bs, l_ds);
[Ab_req_Nmax, F_i_Nmax, ~, ~, ~] = bolt_req( ...
    r_Nmax, W_r, n_Nmax, lc, C_bolt, N_b, sigma_bolt, ...
    'SelectBolt', true, 'Excel', 'Conrod.xlsx', 'Sheet', 3, 'Area', 'Stress');
[t_capr_Nmax, ~] = cap_req(F_i_Nmax, d_breq_Nmax, d_boreq, sigma_yp);

% Absorption = (threshold_new - threshold_base) / (threshold_base * deterioration)
% E1 -- t  E2 -- d_big  E3 -- d_small  E4 -- Ab_bolt  E5 -- t_cap
Absorption_N = [(t_req_Nmax    - t_req)   / (t_req   * Deterioration_N);
                (d_breq_Nmax   - d_breq)  / (d_breq  * Deterioration_N);
                (d_sreq_Nmax   - d_sreq)  / (d_sreq  * Deterioration_N);
                (Ab_req_Nmax   - Ab_req)  / (Ab_req  * Deterioration_N);
                (t_capr_Nmax   - t_capr)  / (t_capr  * Deterioration_N)];

% ── Absorption for P_max ────────────────────────────────────────────────
% FIX: same pattern — compute thresholds at P_max_max, N held at baseline.
F_p_Pmaxmax = pi*(D_p.^2)/4 .* P_max_max;
[t_req_Pmax, ~,~,~,~,~, n_Pmax, r_Pmax] = t_req_rankine(F_p_Pmaxmax, sigma_yp, S, K, lc, A_t, Ixx_coeff, H_R, B_R, N, N_prime, l_s, rho);
[d_breq_Pmax, ~] = big_end(F_p_Pmaxmax, P_bb, l_db);
[d_sreq_Pmax, ~] = small_end(F_p_Pmaxmax, P_bs, l_ds);
[Ab_req_Pmax, F_i_Pmax, ~, ~, ~] = bolt_req( ...
    r_Pmax, W_r, n_Pmax, lc, C_bolt, N_b, sigma_bolt, ...
    'SelectBolt', true, 'Excel', 'Conrod.xlsx', 'Sheet', 3, 'Area', 'Stress');
[t_capr_Pmax, ~] = cap_req(F_i_Pmax, d_breq_Pmax, d_boreq, sigma_yp);

Absorption_Pmax = [(t_req_Pmax  - t_req)  / (t_req  * Deterioration_Pmax);
                   (d_breq_Pmax - d_breq) / (d_breq * Deterioration_Pmax);
                   (d_sreq_Pmax - d_sreq) / (d_sreq * Deterioration_Pmax);
                   (Ab_req_Pmax - Ab_req) / (Ab_req * Deterioration_Pmax);
                   (t_capr_Pmax - t_capr) / (t_capr * Deterioration_Pmax)];
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
